...
$symbols = mysql_query("SELECT symbol, id FROM symbols WHERE exchange=\"amex\"");
...
while ($row = mysql_fetch_array($symbols)) {
  ...
  if(!is_null($todays_data->query->results)){  
    ...
    $insert_query="INSERT INTO amex_quotes (symbolId, date, open, close, high, low, volume, previousClose) " .  
    ...
    print "\nQuery failed. Symbol: " . $row['symbol']; 
  }
...
$fh=fopen($logFile, 'a');
fwrite($fh, "AMEX quotes executed at: " . date("D, m d Y H:i:s") . "\n");
fclose($fh);
