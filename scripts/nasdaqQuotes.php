...
$symbols = mysql_query("SELECT symbol, id FROM symbols WHERE exchange=\"nasdaq\"");
...
while ($row = mysql_fetch_array($symbols)) {
  ...
  if(!is_null($todays_data->query->results)){  
    ...
    $insert_query="INSERT INTO nasdaq_quotes (symbolId, date, open, close, high, low, volume, previousClose) " .  
...
$fh=fopen($logFile, 'a');
fwrite($fh, "NASDAQ quotes executed at: " . date("D, m d Y H:i:s") . "\n");
fclose($fh);
