...
public class EClientSocket {
 ...
 return m_reader;
  ...
  protected void send(int val) {
        send( String.valueOf( val) );
  }

  protected void send(char val) {
        m_dos.write( val);
        sendEOL();
  }

  protected void send(double val) {
        send( String.valueOf( val) );
  }

  protected void send(long val) {
        send( String.valueOf( val) );
  }
  ...
  protected void send(boolean val) {
        send( val ? 1 : 0);
  }
